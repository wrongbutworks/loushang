...
class Runner:
    def __init__(self) -> None:
        ...
        self.controller: ExtensionRuntimeController | None = None
...
def test_extension_runtime_controller_binds_reloads_and_refreshes_with_guard() -> None:
    ...
    controller = ExtensionRuntimeController(
        extension_runner=runner,
        build_bindings=lambda: {"binding": len(runner.bindings) + len(runner.refresh_bindings)},
    ...
    assert runner.invalidations == ["Extension context is stale after extension reload."]
...
def test_extension_runtime_controller_records_bind_failures() -> None:
    ...
    controller = ExtensionRuntimeController(
        extension_runner=BrokenRunner(),
        build_bindings=lambda: object(),
        session_start_event=SessionStartEvent(reason="startup"),
        refresh_resources=lambda: None,
        record_runtime_diagnostic=lambda diagnostic: diagnostics.append(diagnostic.code),
        sync_extension_diagnostics=lambda *, phase: None,
    )
...
def test_extension_runtime_controller_records_session_start_emit_failures() -> None:
    ...
    controller = ExtensionRuntimeController(
        ...
        session_start_event=SessionStartEvent(reason="startup"),
        refresh_resources=lambda: None,
        record_runtime_diagnostic=lambda diagnostic: diagnostics.append((diagnostic.code, diagnostic.message)),
        sync_extension_diagnostics=lambda *, phase: diagnostics_syncs.append(phase),
    )
    ...
    assert runner.bindings == [{"binding": 0}]
    assert diagnostics == [
        ("extension_session_start_failed", "Extension hook 'session_start' failed: start transport boom")
    ]
...
def test_extension_runtime_controller_records_session_refresh_emit_failures() -> None:
    ...
    assert controller.is_refreshing is False
    assert diagnostics == [
        ("extension_session_refresh_failed", "Extension hook 'session_refresh' failed: refresh transport boom")
    ]
    assert diagnostics_syncs == ["runtime"]
...
def test_extension_runtime_controller_records_refresh_runtime_failures() -> None:
    ...
    assert controller.is_refreshing is False
    assert diagnostics == [("extension_runtime_refresh_failed", "Extension runtime refresh failed: refresh runtime boom")]
...
def test_extension_runtime_controller_refresh_bindings_records_failures() -> None:
    ...
    assert runner.refresh_bindings == []
    assert diagnostics == [("extension_runtime_refresh_failed", "Extension runtime refresh failed: refresh bindings boom")]
