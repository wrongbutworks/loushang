...
def test_widgets_datagrid_column_chooser_overlay_example_uses_surface_host() -> None:
    namespace = runpy.run_path("examples/tui/62_widgets_datagrid_column_chooser_overlay.py", run_name="__test__")
    ...
    assert tui.handle_input(InputEvent(kind="text", text="c")) == ()
    assert len(tui.surface_host.entries) == 1
    assert grid.focused is False

    overlay_lines = plain_lines(tui, width=96, height=16)
    assert "Column chooser" in "\n".join(overlay_lines)
    assert any("[x] Symbol" in line for line in overlay_lines)

    assert tui.handle_input(InputEvent(kind="key", key="down")) == ()
    assert tui.handle_input(InputEvent(kind="key", key="space")) == ()
    assert next(column for column in grid.columns if column.key == "price").hidden is True

    assert tui.handle_input(InputEvent(kind="key", key="escape"))[0].kind == "surface_close"
    assert tui.surface_host.entries == []
    assert grid.focused is True
