...
def is_usable_model_selection(selection: object | None) -> bool:
    ...
    if normalized is None:
        return False
    return _is_usable_value(normalized.provider) and _is_usable_value(
        normalized.model_id
    )
...
def model_label_from_selection(selection: object | None) -> str | None:
    ...
    if (
        normalized is None
        or not _is_usable_value(normalized.provider)
        or not _is_usable_value(normalized.model_id)
    ):
        return None
...
async def ensure_usable_session_model(session: Any) -> ModelSelection | None:
    ...
    for candidate in candidates:
        ...
        return normalize_model_selection(updated) or normalize_model_selection(
            candidate
        )
    return None
...
def _matches_preferred_model_detail(
    ...
    return (
        ...
        and endpoint_id == preferred.endpoint_id
        and model_id == preferred.model_id
    )
