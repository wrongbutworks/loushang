...
from loushang.harness.context import validate_summary

SummaryType = Literal["compaction", "branch"]

_FILE_OPERATION_CAPTURE_RE = re.compile(
    r"<(?P<tag>read-files|modified-files)>\s*(?P<body>.*?)\s*</(?P=tag)>", re.DOTALL
)
...
class SummaryQualityReport:
    ...
    def ok(self) -> bool:
        return (
            ...
            and not self.placeholder_sections
        )
...
class SummaryEvaluationResult:
    case_name: str
    quality_report: SummaryQualityReport = field(
        default_factory=lambda: SummaryQualityReport(summary_type="compaction")
    )
...
def validate_summary_contract(
    ...
    *,
    ...
    profile = (
        ...
        else CODING_BRANCH_SUMMARY_PROFILE
    ...
    return SummaryQualityReport(
        ...
        empty_sections=validation.empty_sections,
        placeholder_sections=validation.placeholder_sections,
    )
...
def evaluate_summary_case(case: SummaryEvaluationCase) -> SummaryEvaluationResult:
    ...
    return SummaryEvaluationResult(
        ...
        missing_read_files=_missing_files(
            file_operations["read-files"], case.expected_read_files
        ),
        missing_modified_files=_missing_files(
            file_operations["modified-files"], case.expected_modified_files
        ),
    )


def evaluate_summary_cases(
    cases: list[SummaryEvaluationCase] | tuple[SummaryEvaluationCase, ...],
) -> SummaryEvaluationSuiteResult:
    return SummaryEvaluationSuiteResult(
        results=tuple(evaluate_summary_case(case) for case in cases)
    )
...
def load_summary_evaluation_cases(
    ...
    return tuple(
        _summary_evaluation_case_from_mapping(raw_case) for raw_case in raw_cases
    )
...
def _summary_evaluation_case_from_mapping(value: object) -> SummaryEvaluationCase:
    ...
    return SummaryEvaluationCase(
        ...
        expected_read_files=_string_tuple(
            value.get("expected_read_files", ()), "expected_read_files"
        ),
        expected_modified_files=_string_tuple(
            value.get("expected_modified_files", ()), "expected_modified_files"
        ),
    )
...
def _missing_phrases(
    summary: str, required_phrases: tuple[str, ...]
) -> tuple[str, ...]:
    normalized_summary = summary.lower()
    return tuple(
        phrase
        for phrase in required_phrases
        if phrase.lower() not in normalized_summary
    )


def _missing_files(
    actual_files: tuple[str, ...], expected_files: tuple[str, ...]
) -> tuple[str, ...]:
