...
from loushang.coding.workflow.events import EventPattern
...
def resolve_workflow_files(cwd: str | Path, workflow_path: str | Path) -> tuple[Path, ...]:
...
def _workflow_from_payload(payload: object, *, default_name: str) -> Workflow:
    ...
    steps = tuple(_step_from_payload(item, index=index) for index, item in enumerate(raw_steps, start=1))
    if not steps:
...
def _prompt_step_from_payload(payload: Mapping[object, object], *, index: int) -> PromptStep:
...
def _wait_for_step_from_payload(payload: Mapping[object, object], *, index: int) -> WaitForStep:
    ...
    return WaitForStep(event=raw_event, timeout_s=5.0 if timeout_s is None else timeout_s)


def _wait_step_from_payload(payload: Mapping[object, object], *, index: int) -> WaitStep:
...
def _text_step_from_payload(payload: Mapping[object, object], *, key: str, index: int) -> SteerStep | FollowUpStep:
...
def _expect_step_from_payload(payload: Mapping[object, object], *, index: int) -> ExpectStep:
    raw_expect = _nested_action_payload(payload, "expect")
    return ExpectStep(expect=_workflow_expectation_from_payload(raw_expect, index=index))
...
def _nested_action_payload(payload: Mapping[object, object], key: str) -> object:
    ...
    if raw is not None and raw != {}:
        return raw
    return {str(item_key): item_value for item_key, item_value in payload.items() if item_key != key}
...
def _expectation_from_payload(payload: object, *, index: int) -> StepExpectation:
    ...
    return StepExpectation(
        ...
        command=_command_from_payload(command_payload, index=index) if command_payload is not None else None,
...
def _workflow_expectation_from_payload(payload: object, *, index: int) -> WorkflowExpectation:
    if payload is None:
        payload = {}
    if not isinstance(payload, Mapping):
        raise ValueError(f"workflow step {index} expect must be an object")
    ...
    return WorkflowExpectation(
        events=_event_patterns_from_payload(payload.get("events"), index=index, field="events"),
        not_events=_event_patterns_from_payload(payload.get("not_events"), index=index, field="not_events"),
        queue=queue,
...
def _event_patterns_from_payload(payload: object, *, index: int, field: str) -> tuple[EventPattern, ...]:
    if payload is None:
        return ()
    ...
    for item in payload:
        ...
        if not isinstance(raw_event, str) or not raw_event.strip():
            raise ValueError(f"workflow step {index} {field} entries must contain event")
...
def _parse_yaml_mapping(lines: list[tuple[int, str]], index: int, indent: int) -> tuple[dict[str, object], int]:
...
def _parse_yaml_list(lines: list[tuple[int, str]], index: int, indent: int) -> tuple[list[object], int]:
    ...
    while index < len(lines):
        ...
        if not item_text:
            ...
            item_mapping: dict[str, object] = {key: _parse_yaml_scalar(value_text) if value_text else None}
            if index < len(lines) and lines[index][0] > indent:
                nested, index = _parse_yaml_block(lines, index, lines[index][0])
                if isinstance(nested, Mapping):
                    item_mapping.update(nested)
                else:
                    raise ValueError("list item mapping cannot contain a nested list at the same level")
            item = item_mapping
        else:
            item = _parse_yaml_scalar(item_text)
            if index < len(lines) and lines[index][0] > indent:
                raise ValueError(f"scalar list item cannot contain nested content: {item_text}")
        items.append(item)
    return items, index
