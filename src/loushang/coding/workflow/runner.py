...
class WorkflowAdapter(Protocol):
    async def run_prompt(self, prompt: str) -> str:
        """Run one prompt and return the assistant text produced by that turn."""
...
class AgentSessionWorkflowAdapter:
    def __init__(self, session: Any) -> None:
        ...
        self._unsubscribe = subscribe(self._record_session_event) if callable(subscribe) else None
    ...
    async def prompt(self, prompt: str, *, hold: bool = False) -> str:
        if hold:
            ...
            self._abort_requested = False
            self._active_prompt_task = asyncio.create_task(self._run_held_prompt(prompt))
            return ""
    ...
    async def _run_session_prompt(self, prompt: str, *, emit_start: bool) -> str:
        ...
        if assistant_text and not _has_assistant_message_event(self._events[before_event_count:]):
            await self._emit(WorkflowEvent(type="assistant.message", text=assistant_text))
    ...
    async def follow_up(self, text: str) -> None:
        await self._queue_text(text, action="follow_up", event_type="queue.follow_up_added")
    ...
    async def _queue_text(self, text: str, *, action: str, event_type: str) -> None:
        method = _streaming_prompt_method(self.session, streaming_behavior="steer" if action == "steer" else "followUp")
        if method is None:
        ...
        try:
            ...
            step_results.append(
                WorkflowStepResult(
                    ...
                    checks=(CheckResult(label="prompt completed", ok=False, detail="timed out"),),
                )
            )
            break
            ...
            step_results.append(
                WorkflowStepResult(
                    ...
                    checks=(CheckResult(label="prompt completed", ok=False, detail=str(error)),),
                )
            )
            break
        checks = _evaluate_step_checks(step, adapter=adapter, assistant_text=assistant_text, cwd=root)
        step_results.append(
            WorkflowStepResult(
                index=index,
                prompt=_step_label(step),
...
def _evaluate_step_checks(
    ...
    *,
    ...
    if isinstance(step, PromptStep):
        return evaluate_expectations(step.expect, assistant_text=assistant_text, cwd=cwd)
...
async def _run_step(
    ...
    *,
    ...
    if isinstance(step, PromptStep):
        timeout_s = step.timeout_s if step.timeout_s is not None else default_step_timeout_s
...
async def _call_adapter_action(adapter: WorkflowAdapter, action: str, text: str) -> None:
...
def _emit_step_start(callback: object | None, *, index: int, total: int, step: WorkflowStep) -> None:
...
def _timeout_error_message(step: WorkflowStep, default_step_timeout_s: float | None) -> str:
    timeout_s: float | None
    if isinstance(step, PromptStep):
        timeout_s = step.timeout_s if step.timeout_s is not None else default_step_timeout_s
