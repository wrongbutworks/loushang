...
def evaluate_expectations(
    ...
    *,
    ...
    if expect.command is not None:
        checks.extend(_command_checks(expect.command, cwd=cwd))
    ...
    for queue_name, expected in expect.queue.items():
        ...
        checks.append(
            CheckResult(
                ...
                ok=actual == expected,
                detail="" if actual == expected else f"expected {expected!r}, got {actual!r}",
            )
        )
    checks.extend(_session_state_checks(expect.session_state, session_state, path=("session_state",)))
    checks.extend(_session_state_checks(expect.session_stats, session_stats, path=("session_stats",)))
    checks.extend(_session_state_checks(expect.context_usage, context_usage, path=("context_usage",)))
    return tuple(checks)
    ...
    *,
    ...
    for key, expected_value in expected.items():
        ...
        if isinstance(expected_value, Mapping):
            checks.extend(_session_state_checks(expected_value, actual_value, path=child_path))
            continue
        ...
        checks.append(
            CheckResult(
                ...
                ok=ok,
                detail="" if ok else f"expected {normalized_expected!r}, got {normalized_actual!r}",
            )
        )
    return checks
...
def _assistant_checks(expect: StepExpectation, assistant_text: str) -> list[CheckResult]:
    checks: list[CheckResult] = []
    ...
    if expect.assistant_contains_any:
        matched = [value for value in expect.assistant_contains_any if value in assistant_text]
        checks.append(
            CheckResult(
                label="assistant contains any",
                ok=bool(matched),
                detail="" if matched else f"none found: {', '.join(repr(value) for value in expect.assistant_contains_any)}",
            )
        )
    ...
    for path_text, expected in expect.files_contain.items():
        ...
        checks.append(
            CheckResult(
                ...
                detail="" if expected in content else f"substring not found: {expected!r}",
            )
        )
    return checks


def _command_checks(expect: CommandExpectation, *, cwd: Path) -> list[CheckResult]:
    try:
        ...
        return [
            CheckResult(
                ...
                ok=False,
                detail=f"timed out after {error.timeout}s: {expect.run}",
            )
        ]
    checks = [
        CheckResult(
            label=f"command exits {expect.exit_code}",
            ok=completed.returncode == expect.exit_code,
            detail="" if completed.returncode == expect.exit_code else f"exit code {completed.returncode}",
        )
    ]
