...
def create_edit_tool_definition(
    *,
    ...
    ops = normalize_edit_operations(operations or (options.operations if options is not None else None))
    ...
    @tool(
        name="edit",
        label="Edit",
        description="Apply exact text replacements to a file in the coding workspace.",
        prompt_snippet="- edit: Apply exact text replacements to a file in the coding workspace.",
    )
    async def edit(
        path: str,
        edits: list[EditEntry],
        *,
        ...
        return AgentToolResult(
            content=[TextPart(type="text", text=f"Applied {len(validated_edits)} edits to {path}")],
            details={
                "path": str(resolved),
...
async def _write_exact_text(path: Path, content: str, *, operations: EditOperations) -> None:
