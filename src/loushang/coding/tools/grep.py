...
def create_grep_tool_definition(
    *,
    ...
    selected_operations = operations or (options.operations if options is not None else None)
    external_tool_policy = normalize_external_tool_policy(
        options.external_tool_policy if options is not None else None,
        allow_download=bool(options.allow_external_tool_downloads) if options is not None else False,
    )
    ...
    use_external_tools = selected_operations is None and external_tools_enabled_for_policy(external_tool_policy)
    ...
    @tool(
        name="grep",
        label="Grep",
        description="Search file contents in the coding workspace.",
        prompt_snippet="- grep: Search file contents for patterns in the coding workspace.",
        prompt_guidelines=(
    ...
    async def grep(
        ...
        *,
        ...
        search_path, base_dir = await _resolve_search_path(path or ".", ctx, operations=ops)
        ...
        rendered_entries = await _render_grep_entries(base_dir, matches, context=context_lines, operations=ops)
        raise_if_operation_aborted(ctx.signal)
...
async def _resolve_search_path(path: str, ctx: ToolContext, *, operations: GrepOperations) -> tuple[Path, Path]:
...
def _effective_limit(limit: int | None) -> int:
    return coerce_int_parameter(limit, field_name="limit", minimum=1) or DEFAULT_GREP_LIMIT
...
async def _search_contents_with_rg(
    ...
    *,
    ...
    if completed.returncode != 0 and not completed.stopped_early:
        raise RuntimeError(completed.stderr.strip() or f"ripgrep exited with code {completed.returncode}")
    return matches, match_limit_reached


async def _resolve_rg_path(external_tool_resolver: ExternalToolResolver | None) -> str | None:
...
async def _search_contents_with_operations(
    ...
    *,
    ...
    ignore_matcher = load_ignore_matcher(search_path) if respect_ignore and not search_is_file else None
    candidates = [search_path] if search_is_file else sorted(
        await resolve_operation(operations.walk_files(search_path)), key=lambda item: str(item)
    )
    ...
    for match in matches:
        if context == 0:
            ...
            rendered_entries.append(
                {
                    "rendered": _render_grep_line({**match, "line": rendered_line}, is_match=True),
                    "match": match,
...
def _visible_grep_matches(rendered_entries: list[dict[str, Any]], content: str) -> list[dict[str, Any]]:
...
def _append_grep_notices(
    ...
    *,
    ...
    if match_limit_reached:
        notices.append(f"{match_limit} matches limit reached. Use limit={match_limit * 2} for more, or refine pattern")
    if byte_truncated:
