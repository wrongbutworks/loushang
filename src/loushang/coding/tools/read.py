...
def create_read_tool_definition(
    *,
    ...
    ops = normalize_read_operations(operations or (options.operations if options is not None else None))
    ...
    @tool(
        ...
        prompt_snippet="- read: Read text files and images from the coding workspace.",
    )
    async def read(
        path: str,
        ...
        *,
        ...
        raise_if_operation_aborted(ctx.signal)
        mime_type = await _detect_supported_image_mime_type(resolved, payload, operations=ops)
        if mime_type is not None:
            ...
            if exceeds_inline_limits and not omit_for_model and auto_resize_images:
                ...
                if resize_result is not None:
                    ...
                    dimensions = resize_result.dimensions or _detect_image_dimensions(mime_type, payload)
            ...
            if exceeds_inline_limits or omit_for_model:
                ...
                text_note = _append_non_vision_image_note(text_note, model_supports_image_input)
                return AgentToolResult(
                    ...
                    details={
                        ...
                        "original_width": original_dimensions[0] if original_dimensions is not None else None,
                        "original_height": original_dimensions[1] if original_dimensions is not None else None,
            ...
            return AgentToolResult(
                content=[
                    ...
                    ImagePart(type="image", data=encoded.decode("ascii"), mime_type=mime_type),
                ],
                details={
                    "path": str(resolved),
                    "is_image": True,
        ...
        sliced_text, start_line, end_line, truncated, total_lines, user_limit_remaining = _slice_text(
            text,
            offset=offset,
            limit=limit,
        )
...
async def _resolve_path(path: str, ctx: ToolContext, *, operations: ReadOperations) -> Path:
...
async def _detect_supported_image_mime_type(path: Path, payload: bytes, *, operations: ReadOperations) -> str | None:
...
def _detect_gif_dimensions(payload: bytes) -> tuple[int, int] | None:
    if len(payload) < 10 or not (payload.startswith(b"GIF87a") or payload.startswith(b"GIF89a")):
        return None
...
def _image_exceeds_inline_limits(encoded: bytes, dimensions: tuple[int, int] | None) -> bool:
...
def image_exceeds_inline_limits(encoded: bytes, dimensions: tuple[int, int] | None) -> bool:
            ...
            with Image.open(BytesIO(payload)) as opened_image:
                ...
                try:
                    for target_dimensions in _iter_resize_dimensions(
                        ...
                        try:
                            resized_image.thumbnail(target_dimensions, _pillow_lanczos_filter(Image))
                            resized_dimensions = (resized_image.width, resized_image.height)
...
def _append_non_vision_image_note(text: str, model_supports_image_input: bool | None) -> str:
...
def _slice_text(
    ...
    *,
    ...
    if start_index >= total_lines and offset is not None and not (total_lines == 0 and start_index == 0):
    ...
    return "".join(sliced), start_line, end_line, truncated, total_lines, user_limit_remaining


def _first_line_too_large_notice(text: str, *, start_line: int, path: str) -> str | None:
    if not text:
        return None
...
def _align_rendered_line_range(
    *,
    ...
    visible_lines = _count_visible_rendered_lines(sliced_text=sliced_text, rendered_text=rendered_text)
    if visible_lines == 0:
