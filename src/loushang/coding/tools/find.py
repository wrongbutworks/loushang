...
def create_find_tool_definition(
    *,
    ...
    @tool(
        name="find",
        label="Find",
        description="Find file paths in the coding workspace.",
        prompt_snippet="- find: Find file paths by glob pattern in the coding workspace.",
        prompt_guidelines=(
...
async def _resolve_directory(path: str, ctx: ToolContext, *, operations: FindOperations) -> Path:
...
def _effective_limit(limit: int | None) -> int:
    return coerce_int_parameter(limit, field_name="limit", minimum=1) or DEFAULT_FIND_LIMIT
...
async def _walk_matching_paths_with_fd(
    ...
    *,
    ...
    if fd_path is None:
        ...
        return None
    effective_pattern = pattern if any(char in pattern for char in "*?[]") else f"*{pattern}*"
    command = [
        fd_path,
        "--glob",
        "--color=never",
    ...
    if "/" in effective_pattern:
        ...
        if not effective_pattern.startswith("/") and not effective_pattern.startswith("**/") and effective_pattern != "**":
    ...
    if completed.returncode not in {0, 1} and not completed.stdout:
        raise RuntimeError(completed.stderr.strip() or f"fd exited with code {completed.returncode}")
...
async def _resolve_fd_path(external_tool_resolver: ExternalToolResolver | None) -> str | None:
...
async def _walk_matching_paths_with_operations(
    ...
    *,
    ...
    candidates = sorted(await resolve_operation(operations.walk_files(root)), key=lambda item: str(item))
...
def _path_matches_pattern(relative_path: str, pattern: str) -> bool:
    if any(char in pattern for char in "*?[]"):
        ...
        return (
            ...
            or (zero_depth_pattern is not None and fnmatch.fnmatch(relative_path, zero_depth_pattern))
            or fnmatch.fnmatch(relative_path.rsplit("/", 1)[-1], pattern)
        )
    return pattern in relative_path


def _visible_find_matches(matches: list[dict[str, str]], content: str) -> list[dict[str, str]]:
...
def _append_find_notices(
    ...
    *,
    ...
    if result_limit_reached:
        notices.append(f"{result_limit} results limit reached. Use limit={result_limit * 2} for more, or refine pattern")
    if byte_truncated:
