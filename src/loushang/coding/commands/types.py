...
BUILTIN_SLASH_COMMANDS: tuple[BuiltinSlashCommand, ...] = (
    ...
    BuiltinSlashCommand(
        name="copy", description="Copy an assistant message to clipboard"
    ),
    ...
    BuiltinSlashCommand(
        name="terminal",
        description="Show terminal capabilities and protocol diagnostics",
    ),
    BuiltinSlashCommand(
        name="tools", description="Show or update active tools for this session"
    ),
    BuiltinSlashCommand(
        name="extensions", description="Show loaded extensions and diagnostics"
    ),
