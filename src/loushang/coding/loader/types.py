...
class ResourceBundle:
    ...
    def merge(
        ...
        *,
        ...
        merged_prompt_descriptors = [*self.prompt_descriptors, *(prompt_descriptors or []), *merged_prompts]
        return ResourceBundle(
            cwd=self.cwd,
            agents_path=self.agents_path,
            agents_md=self.agents_md,
            prompt_fragments=[descriptor.text for descriptor in merged_prompt_descriptors if descriptor.enabled],
...
class ResourceSnapshot:
    ...
    def to_bundle(self) -> ResourceBundle:
        ...
        ordered_prompt_descriptors.extend(sorted(self.active_prompt_descriptors, key=_prompt_bundle_sort_key))
        agents_descriptor = self.active_agents_descriptor or _nearest_context_descriptor(context_descriptors)
        active_prompts = [descriptor for descriptor in ordered_prompt_descriptors if not _is_context_prompt(descriptor)]
        return ResourceBundle(
            cwd=self.cwd,
            agents_path=agents_descriptor.source_path if agents_descriptor is not None else None,
            agents_md=agents_descriptor.text if agents_descriptor is not None else None,
            prompt_fragments=[descriptor.text for descriptor in ordered_prompt_descriptors if descriptor.enabled],
...
def _prompt_bundle_sort_key(descriptor: PromptFragmentDescriptor) -> tuple[int, int, str, str]:
    return (
