...
def apply_edits_to_normalized_content(
    ...
    *,
    ...
    initial_matches = [
        fuzzy_find_text(normalized_content, edit["oldText"])
    ...
    for index, edit in enumerate(normalized_edits):
        ...
        if not match.found:
            raise ValueError(
                f"edits[{index}] in {path} did not match any content: {old_text!r}"
            )
        if _count_occurrences(base_content, old_text) > 1:
            raise ValueError(
                f"edits[{index}] in {path} matched more than once: {old_text!r}"
            )
        planned.append(
            (match.index, match.index + match.match_length, index, edit["newText"])
        )
...
def compute_edits_diff_for_content(
    path: str, content: str, edits: list[EditEntry]
) -> DiffStringResult:
    stripped = strip_bom(content)
