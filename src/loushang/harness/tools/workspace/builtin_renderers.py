...
def render_bash_call(
    args: object, theme: Mapping[str, str], context: ToolRenderContext
) -> ToolRenderOutput:
    del theme
    ...
    timeout_suffix = (
        f" (timeout {_format_number(timeout)}s)" if timeout is not None else ""
    )
...
def render_bash_result(
    ...
    rendered = _render_result_with_limit(
        ...
        else DEFAULT_TOOL_OUTPUT_PREVIEW_LINES,
        tail=True,
    )
    started_at = context.state.get("started_at")
    if isinstance(started_at, int | float):
        if not options.is_partial or context.is_error:
            context.state["ended_at"] = (
                context.state.get("ended_at") or time.monotonic()
            )
        end_at = (
            time.monotonic() if options.is_partial else context.state.get("ended_at")
        )
        if isinstance(end_at, int | float):
            label = "Elapsed" if options.is_partial else "Took"
            rendered = _join_non_empty(
                rendered, f"{label} {_format_duration(end_at - started_at)}"
            )
    return rendered
...
def render_edit_result(
    ...
    return render_default_tool_result(
        ...
        ToolRenderResultOptions(
            expanded=context.expanded, is_partial=context.is_partial
        ),
        {},
        context,
    )
...
def _edit_count(mapping: Mapping[str, Any]) -> int | None:
    ...
    if isinstance(edits, list):
        return len(edits)
    if isinstance(mapping.get("oldText"), str) and isinstance(
        mapping.get("newText"), str
    ):
        return 1
    return None
...
def _render_result_with_limit(
    ...
    *,
    ...
    extras = [
        ...
        *(f"[Full output: {path}]" for path in presentation.artifact_paths),
    ]
