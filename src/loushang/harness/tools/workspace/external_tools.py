...
class LocalExternalToolResolver:
    ...
    def resolve_tool(self, name: ExternalToolName) -> str | None:
        managed_path = _managed_binary_path(
            _external_tools_dir(self.tools_dir), name, platform_name=sys.platform
        )
...
class DownloadingExternalToolResolver:
    ...
    _cache: dict[ExternalToolName, str] = field(
    ...
    async def resolve_tool(self, name: ExternalToolName) -> str | None:
        ...
        if not self.allow_download or self.downloader is None:
            return None
        try:
            downloaded_path = await resolve_maybe_awaitable(
                self.downloader.download_tool(name)
            )
        except Exception:
    ...
    def download_tool(self, name: ExternalToolName) -> str | None:
        ...
        with _ExternalToolInstallLock(
            ...
            release = transport.get_latest_release(
                config.repo,
                user_agent=f"{self.app_name}-workspace-tools",
            ...
            try:
                ...
                if extracted_binary is None:
                    raise FileNotFoundError(
                        f"Binary not found in archive: expected {binary_path.name}"
                    )
...
def get_managed_external_tool_install(
    ...
    *,
    ...
    binary_path = _managed_binary_path(
        resolved_tools_dir, name, platform_name=resolved_platform
    )
...
def default_external_tools_dir() -> Path:
    configured_bin_dir = os.environ.get("LOUSHANG_WORKSPACE_TOOLS_DIR")
    if configured_bin_dir:
        return Path(configured_bin_dir).expanduser()
    legacy_bin_dir = os.environ.get("LOUSHANG_CODING_BIN_DIR")
    if legacy_bin_dir:
        return Path(legacy_bin_dir).expanduser()
    legacy_agent_dir = os.environ.get("LOUSHANG_CODING_AGENT_DIR")
    if legacy_agent_dir:
        return Path(legacy_agent_dir).expanduser() / "bin"
    legacy_default = Path.home() / ".loushang" / "coding" / "bin"
    if legacy_default.exists():
        return legacy_default
    return Path.home() / ".loushang" / "tools" / "bin"
...
def _managed_binary_path(
    tools_dir: Path, name: ExternalToolName, *, platform_name: str
) -> Path:
...
class _ExternalToolInstallLock:
    def __init__(
        self, path: Path, *, timeout_seconds: float, stale_seconds: float | None
    ) -> None:
        self.path = path
    ...
    def __enter__(self) -> "_ExternalToolInstallLock":
        ...
        while True:
            try:
                ...
                if _remove_stale_install_lock(
                    self.path, stale_seconds=self.stale_seconds
                ):
                    continue
                if time.monotonic() >= deadline:
                    raise TimeoutError(
                        f"Timed out waiting for external tool install lock: {self.path}"
                    )
                time.sleep(0.01)
