        ...
        if part_type == "text":
            ...
            if isinstance(text, str):
                text_blocks.append(
                    ...
                    if preserve_ansi
                    else normalize_display_text(text)
                )
        elif part_type == "image" and not show_images:
            mime_type = (
                _part_value(part, "mime_type")
                or _part_value(part, "mimeType")
                or "image/unknown"
            )
...
def render_tool_result_presentation(
    ...
    *,
    ...
    body = get_tool_text_output(
        content, show_images=show_images, preserve_ansi=preserve_ansi
    )
...
def _tool_result_notices(details: object | None) -> list[str]:
    ...
    max_bytes = (
        _mapping_int(truncation, "maxBytes")
        or _mapping_int(truncation, "max_bytes")
        or _mapping_int(details, "max_bytes")
    )
