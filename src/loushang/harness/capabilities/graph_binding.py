...
class RuntimeCapabilityGraphBinder:
    ...
    async def bind(
        ...
        async with runtime._binding_lock:
            ...
            try:
                ...
                inventory = _registration_inventory(runtime, candidate, snapshot)
            except asyncio.CancelledError:
                rollback_codes = tuple(
                    (
                        *await _cleanup_scopes(tuple(reversed(constructing_scopes))),
                        *await _cleanup_nodes(tuple(reversed(staged))),
                    )
                )
                runtime._last_attempt = CapabilityGraphBindingAttempt(
                    attempt_number=attempt_number,
            ...
            runtime._registration_inventory = inventory
            replaced = tuple(
    ...
    async def dispose(self, runtime: RuntimeCapabilityGraphRuntime) -> tuple[str, ...]:
        ...
        if not isinstance(runtime, RuntimeCapabilityGraphRuntime):
            raise TypeError("graph Binder requires RuntimeCapabilityGraphRuntime")
        async with runtime._binding_lock:
            if runtime._closed and not runtime._retired_nodes:
            ...
            retiring = (*nodes, *tuple(runtime._retired_nodes))
            runtime._last_attempt = CapabilityGraphBindingAttempt(
                attempt_number=runtime._next_attempt_number(),
                state="disposed",
                target_generation=runtime.generation,
            ...
            cleanup_task = asyncio.create_task(_cleanup_nodes_once(retiring))
            try:
                ...
                raise
            runtime._retired_nodes = [
                mounted for mounted in retiring if not _node_cleanup_complete(mounted)
            ]
            empty_inventory = _empty_registration_inventory(runtime)
            runtime._registration_inventory = empty_inventory
            runtime._last_attempt = CapabilityGraphBindingAttempt(
                attempt_number=runtime._attempt_number,
                state="disposed",
                target_generation=runtime.generation,
    ...
    entries = tuple(
        sorted(
            (
                ...
                for capability_id in sorted(mounted_nodes)
                for owner, identity, state in mounted_nodes[
                    capability_id
                ].registration_scope.inventory
            ),
            key=lambda entry: (
                entry.owner_id,
                entry.surface,
    ...
    revision = _fingerprint(
        {
            ...
            "graph_id": runtime.graph_id,
            "mount_generation": snapshot.generation,
            "schema_version": 1,
        }
    )
...
def _empty_registration_inventory(
    runtime: RuntimeCapabilityGraphRuntime,
) -> RegistrationInventorySnapshot:
    revision = _fingerprint(
        {
...
async def _cleanup_nodes(
    ...
    return await _await_cancellation_atomic(
        asyncio.create_task(_cleanup_nodes_once(nodes))
    )
...
async def _cleanup_scopes(
    scopes: tuple[RegistrationScope, ...],
) -> tuple[str, ...]:
    return await _await_cancellation_atomic(
        asyncio.create_task(_cleanup_scopes_once(scopes))
    )
...
def _node_cleanup_complete(mounted: _MountedCapability) -> bool:
    return mounted.registration_scope.state in {"disposed", "failed_terminal"} and (
        mounted.provider_released
    )
