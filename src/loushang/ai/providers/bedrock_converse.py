...
class BedrockConverseProvider:
    ...
    async def _stream_raw_parts(self, model, context, options) -> AsyncIterator[dict]:
        def _pairing_mode() -> PairingMode:
            if options is None:
                return "repair"
            return "strict" if getattr(options, "pairing_mode", "repair") == "strict" else "repair"
        ...
        try:
            response = await client.post(url, content=headers.pop("_body"), headers=headers)
        ...
        yield {"type": "response_start", "response_id": response.headers.get("x-amzn-requestid", "")}
        ...
        yield {"type": "stop_reason", "stop_reason": _map_stop_reason(payload.get("stopReason"))}
...
def _build_converse_body(model, normalized: dict, resolved, options) -> dict[str, Any]:
    ...
    inference_config: dict[str, Any] = {
        "maxTokens": resolve_output_token_budget(model, resolved, options).value
    }
...
def _build_bedrock_messages(normalized: dict) -> list[dict[str, Any]]:
    ...
    for message in normalized.get("messages", []):
        if isinstance(message, UserMessage):
            messages.append({"role": "user", "content": [{"text": _text_content(message.content)}]})
        elif isinstance(message, AssistantMessage):
            messages.append({"role": "assistant", "content": [{"text": _assistant_text(message)}]})
    if not messages:
