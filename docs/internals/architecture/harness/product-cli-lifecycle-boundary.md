...
| Prompt/stdin/file/image input normalization | `harness.host.prompt_input` owns assembly; `harness.tools.workspace.image_payload` owns image MIME, dimensions, limits, and resizing | Coding supplies prompt/file arguments and the image-resize choice | Coding keeps argument parsing and prompt policy; Host and the read tool consume one image-payload policy |
