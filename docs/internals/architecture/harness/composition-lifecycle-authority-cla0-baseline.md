...
| AUTH-07 | Graph construction | `src/loushang/harness/session/model_call.py::SessionModelCallRuntime.__init__` | The sole production construction site creates one private `RuntimeCapabilityGraphRuntime` and one `RuntimeCapabilityGraphBinder` for `harness.model_input`. |
...
| AUTH-09 | Graph capture | `src/loushang/harness/session/model_call.py::SessionModelCallRuntime.bind` | Captures the declared model-input requirement only after a successful bind. Consumer code receives `CapabilityFacetSet`, not the Graph Runtime. |
| AUTH-10 | Graph disposal | `src/loushang/harness/session/model_call.py::SessionModelCallRuntime.dispose` -> `src/loushang/harness/capabilities/graph_binding.py::RuntimeCapabilityGraphBinder.dispose` | Invalidates captured facets and retires current plus retained cleanup state. `tests/harness/capabilities/test_graph_binding.py::test_graph_dispose_retries_retryable_provider_cleanup` |
...
- `RuntimeCapabilityGraphRuntime` and `RuntimeCapabilityGraphBinder`:
  `SessionModelCallRuntime.__init__` only;
- `RuntimeCapabilityGraphProjector`: `SessionModelCallPreparer.__init__` and
  `SessionModelCallRuntime._projector` only;
