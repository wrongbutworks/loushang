...
| `coding.bootstrap` | Product adapter | Coding retains Product activation effects, factories, defaults, paths, and concrete session/runtime construction. `BootstrapActivationRuntime` and the standard Agent activation plan own ordering, structured failure reporting, and rollback; existing resource, package, diagnostics, transcript, model-catalog, prompt, and tool-registration components own their shared leaves. Coding injects its extension loader/policy, capability profiles, legacy pack identifiers, and final Product choices. |
