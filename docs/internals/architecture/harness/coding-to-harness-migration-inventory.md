...
| `coding.session.queue_controller`, removed `coding.session.session_event_bus` | Product adapter | Queue snapshots, `HostInputQueue`, and `TurnInputQueue` live in `loushang.harness.host`; common Session payloads, `OrderedEventBus`, and the single `RuntimeEvent` stream live in `loushang.harness.events`. Coding keeps AI message construction, preflight and delivery policy, logs, and its UI/RPC event projection adapter. |
