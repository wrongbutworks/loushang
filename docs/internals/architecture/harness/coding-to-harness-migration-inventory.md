...
| `coding.compaction.types.Compaction*`, `coding.session.context_usage`, removed `coding.session.compaction_controller` and `coding.session.retry_controller` | Product adapter / compatibility shim | `harness.agent_transcript` owns Agent transcript token extraction/estimation, context usage snapshots and threshold decisions, standard compaction plan/preparation/result/status, checkpoint commit/event lifecycle, overflow guard, retry classification/backoff/cancellation, and retry events. `AgentSession` binds the Harness runtimes directly. Coding compatibility paths preserve public type names. Coding keeps summary/branch prompt text, model calls, extension hook translation, diagnostics wording, settings/default policy, artifact details, and presentation. |
...
| `coding.session.AgentSession`, controllers, `coding.runtime.AgentSessionRuntime` | Product adapter | `harness.session.SessionFacade` now combines standard Product-facing session operations from one Product-supplied `SessionFacadePorts` bundle. `harness.session.SessionOperationRuntime` supplies explicit capability-grouped input, queue, lifecycle, identity, retry, and maintenance calls over the bound `SessionControlPort`; Coding RPC and the standard Coding Channel adapter use it while retaining Coding JSON mapping and task tracking. `harness.agent_transcript.ProductTranscriptSession` owns the generic Product transcript-session wrapper, so Coding's `SessionManager` binds only its runtime/capability profile and exposes Coding-only runtime capabilities. `AgentSession` adapts `SessionFacade` and `harness.session.SessionRuntime` for run, turn, queue, runtime-event sequencing, and abort/idle coordination. It binds `AgentTranscriptNavigationRuntime`, `AgentTranscriptSelectionRuntime`, and `AgentTranscriptCompactionRuntime` directly; the former Coding controller wrappers are removed. `harness.session.AgentSessionInspector` owns common state/context/statistics/fork-candidate observation; Coding retains its Product RPC/HTML/UI projection. `harness.agent_transcript.AgentTranscriptRetryRuntime` owns retry lifecycle directly; Coding binds its settings and overflow classification, and has removed the redundant `RetryController`. `harness.session.ExtensionInputRuntime`, `ExtensionAgentHookRuntime`, and `ExtensionAgentEventRuntime` own generic extension-to-Agent session integration; `harness.extensions.ExtensionSessionRuntime` owns extension binding/refresh/invalidation coordination. `harness.session.SessionLifecycleRuntime` owns the active session new/restore/fork/import/replacement/disposal transaction, Product store/hook ports, and default `at` fork profile. `harness.session.AgentTranscriptSessionRuntime` composes that transaction runtime with `harness.agent_transcript.AgentTranscriptDirectoryRuntime` current-root/all-root catalog reads, index refresh, coalesced scheduling, and current session-reference resolution; `AgentSessionRuntime` binds the facade directly. `harness.agent_transcript` owns standard branch navigation, cancellation, summary transaction/event ordering, context replay, model/thinking persistence and cycling, fork candidates, transcript inspection, context accounting, and compaction/retry maintenance lifecycle. Coding injects its model catalog/auth resolution, `before`/`at` fork profile and resolver, extension lifecycle/tree hooks, branch-summary/compaction prompt and model calls, cwd/session-file acceptance, diagnostics, callbacks, index policy, and presentation. The core Session/Runtime and extension session contract use snake_case; Pi SDK aliases are intentionally removed. Async transcript state and persistence live in `harness.agent_transcript` and `harness.storage`. |
| removed `coding.session.resource_refresh_controller` | Harness profile / Product adapter | `harness.session.SessionResourceRefreshRuntime` owns ordered prepare, reload, optional extension discovery, disabled-skill activation, bundle commit, prompt/tool rebuild, and contained refresh failure routing. Coding binds `DefaultResourceLoader`, roots, settings, current extension runner, diagnostic wording, watcher trigger, and Coding extension-runtime refresh behavior. |
| `coding.event` | Product adapter | The common runtime envelope, ordering, queue/compaction/retry/branch/metadata/package payloads, committed-record fact, strict `RuntimeEventView`, and generic event-selector mechanics live in `loushang.harness.events`. `loushang.channel` carries a completed view as a separate runtime event family. Coding retains its accepted Agent/session dictionary, Pi aliases, view mapping, tool render enrichment, RPC stream shape, and extension/TUI presentation policy. |
...
| `coding.mode` | Product adapter | Print/RPC stay Coding-owned for command tables and legacy display contracts, but their JSON paths subscribe to the common RuntimeEvent stream and consume Coding-created RuntimeEventViews. `channel.JsonlCommandHost` owns the injected strict-JSON input loop, command validation, and dispatch-error isolation; `channel.RemoteUiContext` owns generic headless dialog correlation, timeout, and snapshot mechanics. Coding maps both into its established RPC responses, extension UI request type, tool enrichment, and Product event projection. The adapters retain explicit transport projection for known dataclasses, paths, mappings, lists, and tuples while rejecting cycles, sets, arbitrary objects, `__dict__` discovery, non-finite floats, and `repr()` fallback. |
...
| `coding.package`, `coding.plugin`, `coding.resources`, `coding.skill` | Split candidate | Package source/manifest/materialization, standard roots/layout, registry/resolver, discovery, skill-loading, structured package catalog, scoped source resolution, lifecycle summary, conflict diagnostics, and install/update/remove/uninstall ordering now live under `loushang.harness.resources`. Coding keeps built-in content registration, compatibility convention activation, additional roots, trust/approval policy, settings injection and scope fallback, update-check wording, CLI/RPC projection, and compatibility facades. |
...
favor of the snake_case contract; JSONL wire-field projection is unchanged.
