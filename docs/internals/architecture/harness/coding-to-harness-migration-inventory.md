...
| `coding.session.AgentSession`, controllers, `coding.runtime.AgentSessionRuntime` | Product adapter | `harness.session.SessionFacade` now combines the standard Product-facing session operations over already-bound runtime, transcript, tool, command, command-execution, view, and retry ports. `AgentSession` remains a Coding compatibility adapter over that Facade and `harness.session.SessionRuntime` for run, turn, queue, runtime-event sequencing, and abort/idle coordination. `harness.session.AgentSessionInspector` owns common state/context/statistics/fork-candidate observation; Coding `SessionViewController` retains Pi stats and Product presentation. `harness.session.SessionLifecycleRuntime` owns the active session new/restore/fork/import/replacement/disposal transaction, Product store/hook ports, and default `at` fork profile. `harness.agent_transcript` owns standard branch navigation, cancellation, summary transaction/event ordering, context replay, model/thinking persistence and cycling, fork candidates, transcript inspection, context accounting, and compaction/retry maintenance lifecycle. Coding injects its model catalog/auth resolution, `before`/`at` fork profile and resolver, extension lifecycle/tree hooks, branch-summary/compaction prompt and model calls, cwd/session-file acceptance, diagnostics, callbacks, index policy, and presentation. Coding retains concrete commands, tool defaults/materialization, compaction policy, Product event projection, Pi protocol aliases, lifecycle cleanup, and storage-provider selection. Async transcript state and persistence live in `harness.agent_transcript` and `harness.storage`. |
