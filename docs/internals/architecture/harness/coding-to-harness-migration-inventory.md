...
| `coding.cli` | Product binding | `harness.cli` owns the standard Agent grammar/value projection, two-pass application lifecycle, launch projection, operation pack, and prepared-turn lifecycle; Channel owns streams and disposal. Coding retains additive Method/Work syntax, Product policy, runner bindings, and wording. |
...
| removed `coding.work_shell`, `coding.work_runtime`, `coding.work_executor`, and `work.coding` | Canonical split complete | `loushang.work.session.SessionWorkRuntime` owns reusable Agent-session turn execution over canonical `WorkRuntime`. Coding supplies `CODING_WORK_PROFILE`, its Agent-event fact projection, renderer/failure wording, and Method turn preparation through `coding.domain.work` and the existing HarnessTUI hosts. |
