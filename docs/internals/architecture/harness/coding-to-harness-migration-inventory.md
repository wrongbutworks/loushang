...
| removed `coding.session.queue_controller`, `prompt_controller`, `agent_event_router`, and `session_event_bus` | Harness profile / Product adapter | Queue snapshots, `HostInputQueue`, and `TurnInputQueue` live in `loushang.harness.host`; session queue delivery, prompt ordering, and Agent-event routing live in the optional `loushang.harness.session` Agent/AI profile; common Session payloads, `OrderedEventBus`, and the single `RuntimeEvent` stream live in `loushang.harness.events`. Coding injects AI message construction, preflight, extension, retry, compaction, diagnostics, transcript, delivery policy, and its UI/RPC event projection adapter. |
