...
| `coding.tool_pack` | Product adapter, audited retained | Existing Harness workspace tool factories, contribution resolution, availability, activation, and rebinding already own the mechanisms. Coding keeps default seven-tool membership/order, descriptions and prompt snippets, managed downloader default, and injection of Coding policy, approval, diagnostics, and execution services. Moving that data would move Product policy, while adding another profile engine would duplicate existing owners. |
...
| removed `coding.source_info`; `SourceInfo` consumers | Canonical owner | `SourceInfo`, `SourceScope`, and `SourceOrigin` live in `loushang.harness.resources.source`. Runtime identity collection and formatting live in `loushang.observability.runtime_identity`; Coding supplies labels through `coding.diagnostics.profile`. |
...
| `coding.session.AgentSession`, controllers, `coding.runtime.AgentSessionRuntime` | Product adapter | `harness.session.AgentProductSession` now binds `SessionFacade`, `SessionComposition`, diagnostics, commands, packages, extensions, transcript, retry, compaction, and lifecycle through the existing runtimes. Coding's `AgentSession` is a thin subclass that injects its capability profile, prompts, summary executors, changelog, clipboard, footer, and package-summary policy. The former Coding command/package controllers and other controller wrappers are removed rather than retained as facades. `harness.session.SessionOperationRuntime` supplies capability-grouped input, queue, lifecycle, identity, retry, and maintenance calls over the bound `SessionControlPort`. `harness.agent_transcript.ProductTranscriptSession` owns the generic Product transcript-session wrapper. `harness.session.AgentSessionInspector` owns common state/context/statistics/fork-candidate observation; `harness.agent_transcript.AgentTranscriptRetryRuntime` owns retry lifecycle. `harness.extensions.agent` owns the extension-to-Agent input, hook, and observation-only lifecycle profile. `harness.session.SessionLifecycleRuntime` owns the active session new/restore/fork/import/replacement/disposal transaction; `harness.session.AgentTranscriptSessionRuntime` composes that lifecycle transaction with transcript directory/index and current-reference resolution. Coding CLI, RPC, builtins, and extension replacement consume typed operation results; Pi-style lifecycle aliases are removed. The core Session/Runtime and extension contracts use snake_case. |
...
| `coding.event` | Thin Product adapter | `harness.events.session_types` owns the shared Agent/session dictionary contracts; `harness.events.session_projection` owns standard views, render enrichment, stream shaping, selector mechanics, and snake_case serialization; `harness.events.runtime_views` owns RuntimeEvent-to-view shaping and delivery hints; `harness.events.recording_policy` owns transcript-write and cancellation policy. `loushang.channel` carries completed views as a separate runtime event family. Coding retains only Product/runtime mapping, Work projection, final presentation, and established thin import facades; it no longer emits or accepts Pi/camelCase event aliases. |
...
| removed `coding.mode` | Canonical split complete | `harness.host.rpc` owns the shared JSONL RPC host, standard session-operation handlers, state/model/diagnostic dispatch, task draining, and strict transport projection. `harnesstui.conversation` owns plain/JSON output and Agent presentation binding. `channel` owns framing, correlation, cancellation, and delivery. `work.channel` owns the Work operation binding. Coding injects only its Product plan, Work profile, domain vocabulary, renderer, and diagnostics through CLI/UI composition; no mode facade or Pi alias remains. |
...
| removed `coding.work_shell`, `coding.work_runtime`, `coding.work_executor`, and `work.coding`; collapsed `coding.domain.work` | Canonical split complete | `loushang.work.session.SessionWorkRuntime` owns reusable Agent-session turn execution over canonical `WorkRuntime`; `work.agent_projection` owns standard Agent runtime-event fact projection and its runtime factory; `work.channel` owns Work-to-Channel operation binding. Coding supplies only `CODING_WORK_PROFILE` and the thin factory that selects it. `loushang.method.MethodDomainRuntime` owns discovery/select/compile/project/prompt preparation, while `CodingDomainApp` injects `domain="coding"` and the Product guidance template. |
