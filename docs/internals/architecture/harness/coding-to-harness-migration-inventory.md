...
| `coding.bootstrap` | Product adapter | Coding retains Product activation callbacks, factories, and defaults; `harness.bootstrap.BootstrapActivationRuntime` owns activation ordering, structured failure reporting, and rollback. `harness.bootstrap.register_extension_tools` now owns extension-tool contribution projection, pack composition, conflict filtering, and registry registration; Coding injects its runner, bundle merge, diagnostic type, and legacy pack identifiers. Resource/skill activation, prompt composition, and tool/command pack ordering remain injected Product choices. |
