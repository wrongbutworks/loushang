...
| `coding.event` | Thin Product adapter | `harness.events.session_types` owns the shared Agent/session dictionary contracts. `harness.events.session_projection` owns standard full/compact/assistant-stream/tools/final views, injected tool-render enrichment, stream shaping, selector mechanics, and the recursive snake_case JSON contract. `loushang.channel` carries a completed view as a separate runtime event family. Coding retains only the RuntimeEvent-to-session mapping, Product/work projection, final presentation policy, and a temporary import surface; it no longer emits or accepts Pi/camelCase event aliases. |
