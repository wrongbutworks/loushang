...
| `coding.compaction.types.Compaction*`, `coding.session.context_usage`, `coding.session.compaction_controller`, `coding.session.retry_controller` | Product adapter / compatibility shim | `harness.agent_transcript` owns Agent transcript token extraction/estimation, context usage snapshots and threshold decisions, standard compaction plan/preparation/result/status, checkpoint commit/event lifecycle, overflow guard, retry classification/backoff/cancellation, and retry events. Coding compatibility paths preserve public type names. Coding keeps summary/branch prompt text, model calls, extension hook translation, diagnostics wording, settings/default policy, artifact details, and presentation. |
...
| `coding.session.AgentSession`, controllers, `coding.runtime.AgentSessionRuntime` | Product adapter | `harness.session.SessionFacade` now combines standard Product-facing session operations from one Product-supplied `SessionFacadePorts` bundle. `AgentSession` remains a Coding compatibility adapter over that Facade and `harness.session.SessionRuntime` for run, turn, queue, runtime-event sequencing, and abort/idle coordination. `harness.session.AgentSessionInspector` owns common state/context/statistics/fork-candidate observation; Coding keeps only pure Pi/UI/RPC/HTML projection in `coding.platform.session_projection`, and has removed the redundant `SessionViewController`. `harness.agent_transcript.AgentTranscriptRetryRuntime` owns retry lifecycle directly; Coding binds its settings and overflow classification, and has removed the redundant `RetryController`. `harness.session.SessionLifecycleRuntime` owns the active session new/restore/fork/import/replacement/disposal transaction, Product store/hook ports, and default `at` fork profile. `harness.agent_transcript` owns standard branch navigation, cancellation, summary transaction/event ordering, context replay, model/thinking persistence and cycling, fork candidates, transcript inspection, context accounting, and compaction/retry maintenance lifecycle. Coding injects its model catalog/auth resolution, `before`/`at` fork profile and resolver, extension lifecycle/tree hooks, branch-summary/compaction prompt and model calls, cwd/session-file acceptance, diagnostics, callbacks, index policy, and presentation. Coding retains concrete commands, tool defaults/materialization, compaction policy, Product event projection, Pi protocol aliases, lifecycle cleanup, and storage-provider selection. Async transcript state and persistence live in `harness.agent_transcript` and `harness.storage`. |
...
| `coding.extensions.api`, `runner`, `types`, `policy`, `hooks` | Product adapter | Neutral records, registration, conflict resolution, stable route planning, observer/interceptor/reducer dispatch, resource contribution execution, binding storage/lifetimes, generic bound/unbound runtime contexts, and `harness.extensions.runtime.ExtensionRuntime` composition live in Harness. The runtime owns registry snapshots, commands/flags/shortcuts, generic input/event dispatch, tool wrapping, renderer/diagnostic/visibility projection, and resource discovery through injected context factories. Coding keeps descriptor loading with its API, typed model/thinking/command specialization, provider/UI callback injection, concrete permission defaults, Product result reducers, session decisions, Agent tool-call adaptation, and legacy event mapping. |
...
| `coding.mode` | Product adapter | Print/RPC stay Coding-owned for command tables and Pi display contracts, but their JSON paths subscribe to the common RuntimeEvent stream and consume Coding-created RuntimeEventViews. The adapters retain explicit transport projection for known dataclasses, paths, mappings, lists, and tuples while rejecting cycles, sets, arbitrary objects, `__dict__` discovery, non-finite floats, and `repr()` fallback. |
