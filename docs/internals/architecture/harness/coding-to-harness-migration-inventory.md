...
| removed `coding.source_info`; `SourceInfo` consumers | Canonical owner | `SourceInfo`, `SourceScope`, and `SourceOrigin` live in `loushang.harness.resources.source`. Runtime identity collection and formatting live in `loushang.foundation.observability.runtime_identity`; Coding supplies labels through `coding.diagnostics.profile`. |
