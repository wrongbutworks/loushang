...
- 语义上对齐 `pi` 的 `AgentSessionRuntime`
...
- replacement 顺序对齐 `pi`：`session_before_switch` / `session_before_fork` 通过后，先 `session_shutdown(old)`，再创建新 session 并触发 `session_start(new)`
- `fork_session_with_result(entry_id, position)` 对齐 `pi` 的 fork target 语义：`at` 保留目标 entry，`before` 仅支持 user message 并 fork 到其 parent，同时返回 selected text
- extension command context 的 `ctx.fork(entryId)` 默认使用 `position="before"`；需要 clone 当前 entry 时显式传 `position="at"`
- runtime 提供 snake_case API 和 pi-style camelCase/action API；camelCase/action API 返回 `{cancelled}` / `{cancelled, selectedText}` 这类 pi-style result payload
- `newSession(options)` 支持 pi-style `parentSession` / `setup` / `withSession`；`setup` 必须是 async callable，在新 `SessionManager` 上运行，随后同步 agent messages
- `switchSession(path, options)` 和 `fork(entryId, options)` 支持 pi-style `withSession`，并在 replacement 完成、新 session 已成为 current session 后执行
...
- Python runtime replacement/action alias 也是 async API：`newSession()` / `switchSession()` / `fork()` / `clone()` / `importFromJsonl()` 必须 await；
  `setup` / `withSession` 对齐 pi 的 `Promise<void>` contract，运行时要求 async callable
- `importFromJsonl(path, cwd_override?)` 对齐 `pi` 的 runtime import：把外部 JSONL 复制进当前 `session_dir`，走 `session_before_switch`，再用 resume lifecycle 替换 current session
...
- `restore_session(...)` / `importFromJsonl(...)` / `rename_session(...)` / `delete_session(...)` 的未处理失败会记录 `session_restore_failed` / `session_import_failed` / `session_rename_failed` / `session_delete_failed` runtime diagnostic；调用方仍收到原异常，诊断层提供稳定 code 与 operation details
- `restore_session(...)` / `switch_session(...)` 支持 pi-style session id prefix 解析；多匹配时返回稳定 ambiguous error，避免 CLI/RPC 上层重复实现 session lookup
- `list_all_session_summaries()` / `find_all_session_summaries(query)` 对齐 `pi` 的 all-session lookup 思路：runtime 以当前 session dir 的父目录作为 sessions root，聚合兄弟 project/session 目录中的 JSONL summaries
