...
```bash
uv --cache-dir .uv-cache run --extra dev python -m loushang.coding.testing.tui.runner --list
...
uv --cache-dir .uv-cache run --extra dev python -m loushang.coding.testing.tui.runner --tag composer --json
```
