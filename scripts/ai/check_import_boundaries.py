...
def check_import_boundaries() -> list[str]:
    ...
    provider_modules = {
        path.name for path in (AI_SRC / "providers").glob("*.py")
    }
    forbidden_modules = sorted(provider_modules & DISALLOWED_CORE_PROVIDER_MODULES)
    if forbidden_modules:
        offenders.append(f"core providers contain non-core adapters: {forbidden_modules}")

    for path in sorted(TOP_LEVEL_EXAMPLES.glob("[0-9][0-9]_*.py")):
        relative_path = path.relative_to(REPO_ROOT).as_posix()
        tree = ast.parse(path.read_text(encoding="utf-8"), filename=relative_path)
        for module in _imported_modules(tree):
            if not module.startswith("loushang.ai"):
                continue
            if module not in ALLOWED_TOP_LEVEL_EXAMPLE_IMPORTS:
                offenders.append(f"{relative_path} imports non-stable AI module {module}")
    return offenders
