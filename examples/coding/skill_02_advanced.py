...
---
name: expert
description: Execute advanced refactoring requiring human confirmation
disable-model-invocation: true
---
This skill performs large-scale refactoring across multiple files.
...
async def main() -> None:
    with TemporaryDirectory(prefix="loushang-skill-adv-") as tmpdir:
        project_root = Path(tmpdir)
        skills_dir = project_root / "skills"
        skills_dir.mkdir(parents=True)
        (skills_dir / "review").mkdir(parents=True)
        (skills_dir / "test").mkdir(parents=True)
        (skills_dir / "expert").mkdir(parents=True)
        ...
        runtime = create_agent_session_runtime(
            ...
            system_prompt="Skill advanced demo.",
            persist=False,
        )
        ...
        print("--- System Prompt (available_skills injection) ---")
